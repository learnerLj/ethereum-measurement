...
func ListenV4(c UDPConn, ln *enode.LocalNode, cfg Config) (*UDPv4, error) {
	...
	log.Info("Measure Creating 3.2: setting discV4 table")
	tab, err := newMeteredTable(t, ln.Database(), cfg)
	if err != nil {
		return nil, err
	}
	t.tab = tab
	log.Info("Measure Creating 3.3: setting discV4 table loop")
	go tab.loop()
...
func (t *UDPv4) ping(n *enode.Node) (seq uint64, err error) {
	...
	dbMsg := measurement.DiscvPing{ToIp: n.IP().String(), ToUdp: uint16(n.UDP())}
	uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, n.ID().String(), false, "")
	if err = <-rm.errc; err == nil {
		seq = rm.reply.(*v4wire.Pong).ENRSeq
		p := rm.reply.(*v4wire.Pong)
		dbMsgRes := p.ToDiscvPong()
		measurement.Measure.InsertMsgAddMeta(&dbMsgRes, n.ID().String(), true, uid)
	}
	return seq, err
}
...
func (t *UDPv4) findnode(toid enode.ID, toaddr *net.UDPAddr, target v4wire.Pubkey) ([]*node, error) {
	...
	nodes := make([]*node, 0, originBucketSize)
	dbNodeInfo := make([]*measurement.NodeInfo, 0, originBucketSize)
	nreceived := 0
	rm := t.pending(toid, toaddr.IP, v4wire.NeighborsPacket, func(r v4wire.Packet) (matched bool, requestDone bool) {
		...
		for _, rn := range reply.Nodes {
			...
			dbNodeInfo = append(dbNodeInfo, &measurement.NodeInfo{
				Ip:  n.IP().String(),
				Udp: uint16(n.UDP()),
				Tcp: uint16(n.TCP()),
				Id:  n.ID().String()})
		}
		return true, nreceived >= originBucketSize
	})
	findnodeReq := &v4wire.Findnode{
		Target:     target,
		Expiration: uint64(time.Now().Add(expiration).Unix()),
	}
	dbMsg := findnodeReq.ToDiscvFindnode()
	uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, toid.String(), false, "")
	t.send(toaddr, toid, findnodeReq)
	...
	if err == nil {
		dbMsgRes := measurement.DiscvNeighbors{Neighbors: dbNodeInfo, Timeout: timeout}
		measurement.Measure.InsertMsgAddMeta(&dbMsgRes, toid.String(), true, uid)
	}
	return nodes, err
}
...
func (t *UDPv4) RequestENR(n *enode.Node) (*enode.Node, error) {
	...
	dbMsg := measurement.DiscvENRRequest{}
	uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, n.ID().String(), false, "")
	...
	enrRes := rm.reply.(*v4wire.ENRResponse)
	respN, err := enode.New(enode.ValidSchemes, &enrRes.Record)
	if err != nil {
		return nil, err
	}
	if respN.ID() != n.ID() {
	...
	dbMsgRes := enrRes.ToDiscvENRResponse()
	measurement.Measure.InsertMsgAddMeta(&dbMsgRes, respN.ID().String(), true, uid)
	return respN, nil
}
...
func (t *UDPv4) ensureBond(toid enode.ID, toaddr *net.UDPAddr) {
	...
	if tooOld || t.db.FindFails(toid, toaddr.IP) > maxFindnodeFailures {
		dbMsg := measurement.DiscvPing{
			ToIp:  toaddr.IP.String(),
			ToUdp: uint16(toaddr.Port)}
		uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, toid.String(), false, "")
...
func (t *UDPv4) handlePing(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	req := h.Packet.(*v4wire.Ping)
	dbMsg := req.ToDiscvPing()
	uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, fromID.String(), true, "")
	...
	dbMsgRes := pongRes.ToDiscvPong()
	measurement.Measure.InsertMsgAddMeta(&dbMsgRes, fromID.String(), false, uid)
	// Reply.
	t.send(from, fromID, pongRes)

	// Ping back if our last pong on file is too far in the past.
	n := wrapNode(enode.NewV4(h.senderKey, from.IP, int(req.From.TCP), from.Port))
	if time.Since(t.db.LastPongReceived(n.ID(), from.IP)) > bondExpiration {

		dbMsg2 := measurement.DiscvPing{
			ToIp:  n.IP().String(),
			ToUdp: uint16(n.UDP())}
		uid2, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg2, fromID.String(), false, "")
		rm := t.sendPing(fromID, from, func() {
			t.tab.addVerifiedNode(n)
		})
		if err := <-rm.errc; err == nil {
			p := rm.reply.(*v4wire.Pong)
			dbMsgRes2 := p.ToDiscvPong()
			measurement.Measure.InsertMsgAddMeta(&dbMsgRes2, fromID.String(), true, uid2)
		}
	} else {
		t.tab.addVerifiedNode(n)
	}
...
func (t *UDPv4) handleFindnode(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	...
	dbMsg := measurement.DiscvFindNode{Target: req.Target.ID().String()}
	uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, fromID.String(), true, "")
...
func (t *UDPv4) handleENRRequest(h *packetHandlerV4, from *net.UDPAddr, fromID enode.ID, mac []byte) {
	dbMsg := h.Packet.(*v4wire.ENRRequest).ToDiscvENRRequest()
	uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, fromID.String(), true, "")

	enrRes := &v4wire.ENRResponse{
		ReplyTok: mac,
		Record:   *t.localNode.Node().Record(),
	}
	dbMsgRes := enrRes.ToDiscvENRResponse()
	measurement.Measure.InsertMsgAddMeta(&dbMsgRes, fromID.String(), false, uid)
	t.send(from, fromID, enrRes)
}
