...
// ToDiscvPing converts response to measurement Ping. If
func (m *Ping) ToDiscvPing() measurement.DiscvPing {
	//To's TCP is always zero
	dbMsg := measurement.DiscvPing{
		FromIp:  m.From.IP.String(),
		FromUdp: m.From.UDP,
		FromTcp: m.From.TCP,
		ToIp:    m.To.IP.String(),
		ToUdp:   m.To.UDP}
...
func (m *Pong) ToDiscvPong() measurement.DiscvPong {
	//To's TCP is always zero
	dbMsg := measurement.DiscvPong{
		ToIp:  m.To.IP.String(),
		ToUdp: m.To.UDP,
		ToTcp: m.To.TCP}
	return dbMsg
...
func (m *Findnode) ToDiscvFindnode() measurement.DiscvFindNode {
	return measurement.DiscvFindNode{
		Target: m.Target.ID().String(),
	}
}

func (m *Neighbors) ToDiscvNeighbors() measurement.DiscvNeighbors {
	dbNodeInfo := make([]*measurement.NodeInfo, 0)
	...
	return measurement.DiscvNeighbors{Neighbors: dbNodeInfo}

}

func (m *ENRRequest) ToDiscvENRRequest() measurement.DiscvENRRequest {
	dbMsg := measurement.DiscvENRRequest{}
	return dbMsg
}

func (m *ENRResponse) ToDiscvENRResponse() measurement.DiscvENRResponse {
	n, _ := enode.New(enode.ValidSchemes, &m.Record)
	dbMsgRes := measurement.DiscvENRResponse{
		Ip:  n.IP().String(),
		Id:  n.ID().String(),
		Udp: uint16(n.UDP()),
		Tcp: uint16(n.TCP()),
