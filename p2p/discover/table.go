...
const (
	alpha            = 1000    // Kademlia concurrency factor
	bucketSize       = 1048576 // Kademlia bucket size
...
func newTable(t transport, db *enode.DB, cfg Config) (*Table, error) {
	...
	tab.seedRand()
	log.Info("Measure Creating 3.4: loading seed nodes")
	tab.loadSeedNodes(false)
	log.Info("Measure Creating 3.5: finish seed nodes")
	return tab, nil
}

func newMeteredTable(t transport, db *enode.DB, cfg Config) (*Table, error) {
	tab, err := newTable(t, db, cfg)
	log.Info("Measure Creating 3.3: got discV4 table")
	if err != nil {
		return nil, err
	}
	if metrics.Enabled {
...
func (tab *Table) doRefresh(done chan struct{}) {
	defer close(done)
	log.Trace("Measure refresh table", "number", tab.len())
...
func (tab *Table) loadSeedNodes(isRefresh bool) {
	...
	for i := range seeds {
		...
		if !isRefresh {
			go func() {
				log.Info("Measurement: loadSeedNodes")
				if p := measurement.Measure.GetPeer(seed.ID().String()); p == nil {
					measurement.Measure.InsertPeer(&measurement.Peer{
						Ip:     seed.IP().String(),
						Udp:    uint16(seed.UDP()),
						Tcp:    uint16(seed.TCP()),
						Id:     seed.ID().String(),
