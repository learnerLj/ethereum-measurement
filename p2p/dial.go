	...
	for {
		...
		case node := <-nodesCh:
			if err := d.checkDial(node); err != nil {
				...
				log.Trace("Measure Dialing", "id", node.ID(), "ip", node.IP(), "slots", slots)
...
func (d *dialScheduler) startDial(task *dialTask) {
	node := task.dest()
	dTask := task.toDialTask()
	measurement.Measure.InsertDialTask(dTask)
...
func (t *dialTask) toDialTask() *measurement.DialTask {
	n := t.dest()
	d := measurement.DialTask{
		To:     n.ID().String(),
		Moment: time.Now(),
	}
