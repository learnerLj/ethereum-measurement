...
const (
	...
	// sources.
	discmixTimeout = 1000 * time.Millisecond
...
func (srv *Server) Start() (err error) {
	...
	log.Info("Measure Creating 3: setting discovery")
	if err := srv.setupDiscovery(); err != nil {
		return err
	}
	log.Info("Measure Creating 4: setting dial")
...
func (srv *Server) setupDiscovery() error {
	...
	if srv.DiscoveryV4 {
		log.Info("Measure Creating 3.1: setting discovery V4")
		cfg := discover.Config{
	...
	log.Info("Measure Creating 3.2: adding bootstrap nodes")
...
func (srv *Server) SetupConn(fd net.Conn, flags connFlag, dialDest *enode.Node) error {
	...
	if err != nil {
		if reason, ok := err.(DiscReason); ok {
			...
			dbMsg := measurement.RLPxDisc{DiscReason: uint8(reason)}
			measurement.Measure.InsertMsgAddMeta(&dbMsg, id, c.is(inboundConn), "")
		}
		if !c.is(inboundConn) {
			markDialError(err)
		}
...
func (srv *Server) setupConn(c *conn, dialDest *enode.Node) error {
	...
	if err != nil {
		srv.log.Trace("Measure: Failed RLPx handshake", "addr", c.fd.RemoteAddr(), "conn", c.flags, "err", err)
	...
	//log.Trace("Measure >> "+"RLPxHello", "id", c.node.ID(), "addr", c.node.IP())
	log.Trace("Measure << "+"RLPxHello", "id", c.node.ID(), "addr", c.node.IP(), "err", err)
	dbMsg := srv.ourHandshake.ToRLPxHello(srv.localnode.Node(), isDial)
	uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, c.node.ID().String(), false, "")
	if err != nil {
		clog.Trace("Failed p2p handshake", "err", err)
		return fmt.Errorf("%w: %v", errProtoHandshakeError, err)
	}
	dbMsgRes := phs.ToRLPxHello(c.node, isDial)
	measurement.Measure.InsertMsgAddMeta(&dbMsgRes, c.node.ID().String(), true, uid)
	...
	if err != nil {
		clog.Trace("Measure Rejected peer", "err", err)
		return err
	}
	// update peer's connectivity status
	measurement.Measure.SetPeerConnectionStatus(c.node.ID().String(), true)
	measurement.Measure.UpdateDialTask(c.node.ID().String(), 1)
	log.Trace("Measure RLPx connected ", "id", c.node.ID(), "addr", c.node.IP())
	return nil
}
...
func (srv *Server) launchPeer(c *conn) *Peer {
	...
	go srv.runPeer(p)
	log.Trace("Measure eth launched")
	return p
}
