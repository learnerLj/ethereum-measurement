...
func (p *protoHandshake) ToRLPxHello(n *enode.Node, isDial bool) measurement.RLPxHello {
	m := measurement.RLPxHello{
		ClientId: p.Name,
		Tcp:      uint16(n.TCP()),
		Ip:       n.IP().String(),
		Udp:      uint16(n.UDP()),
	...
	myCaps := make([]measurement.Cap, 0, len(p.Caps))
	for _, v := range p.Caps {
		myCaps = append(myCaps, measurement.Cap{Name: v.Name, Version: v.Version})
	}
	m.Caps = myCaps
	return m
}
...
func (p *Peer) Disconnect(reason DiscReason) {
	...
	case p.disc <- reason:
		...
		measurement.Measure.InsertMsgAddMeta(&dbMsg, p.ID().String(), false, "")
	case <-p.closed:
	}
}
...
loop:
	for {
		...
		case err = <-readErr:
			...
			dbMsg := measurement.RLPxDisc{DiscReason: uint8(reason)}
			measurement.Measure.InsertMsgAddMeta(&dbMsg, p.ID().String(), true, "")
			break loop
		case err = <-p.protoErr:
...
func (p *Peer) pingLoop() {
	...
	for {
		...
		case <-ping.C:
			...
			dbMsg := measurement.RLPxPing{}
			measurement.Measure.InsertMsgAddMeta(&dbMsg, p.ID().String(), false, "")
			ping.Reset(pingInterval)

		case <-p.pingRecv:
			dbMsg := measurement.RLPxPing{}
			uid, _ := measurement.Measure.InsertMsgAddMeta(&dbMsg, p.ID().String(), true, "")
			dbMsgRes := measurement.RLPxPong{}
			measurement.Measure.InsertMsgAddMeta(&dbMsgRes, p.ID().String(), true, uid)
