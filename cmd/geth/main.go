...
func geth(ctx *cli.Context) error {
	...
	defer stack.Close()
	// start measurement
	log.Info("Measure Creating 1")
	measurement.Measure = measurement.NewMeasurement(
		"/media/zihao/disk1/jiahao/workstation/eth-nodes/go-ethereum/measurement/instruct.env")
	startNode(ctx, stack, backend, false)
	s, n := stack.Server(), stack.Server().NodeInfo()
	measurement.Measure.InitNode(s.Logger.New("Measure", ""), s.NodeInfo().ID, n.IP, n.Ports.Listener, n.Ports.Discovery) // add local node interface
	log.Info("Measure Created")
	stack.Wait()
	return nil
}
