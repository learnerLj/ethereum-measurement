...
func (p *Peer) Handshake(network uint64, td *big.Int, head common.Hash, genesis common.Hash, forkID forkid.ID, forkFilter forkid.Filter) error {
	...
	go func() {
		...
		dbMsg := sp.ToEthStatus()
		measurement.Measure.InsertMsgAddMeta(&dbMsg, p.id, false, uid)

		errc <- p2p.Send(p.rw, StatusMsg, sp)
	}()
	go func() {
		errc <- p.readStatus(network, &status, genesis, forkFilter, uid)
	}()
	...
	measurement.Measure.UpdateDialTask(p.id, 2)
	return nil
}

// readStatus reads the remote handshake message.
func (p *Peer) readStatus(network uint64, status *StatusPacket, genesis common.Hash, forkFilter forkid.Filter, uid string) error {
	...
	dbMsg := status.ToEthStatus()
	measurement.Measure.InsertMsgAddMeta(&dbMsg, p.id, true, uid)
